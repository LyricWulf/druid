...
use crate::window::{ChildWindow, ImeUpdateFn, Window};
use crate::{
    Command, Data, Env, Event, Handled, InternalEvent, KeyEvent, Menu, NativeWindowDesc,
...
impl<T: Data> Inner<T> {
    ...
    fn set_native_layout(&mut self, id: WindowId, origin: Option<Point>, size: Option<Size>) {
        ...
        if let Some(win) = self.windows.child_windows.get_mut(&id) {
            ...
            tracing::debug!("Failed to find native window with id={:?}", id);
        }
    }
        ...
        match cmd.target() {
            ...
            _ if cmd.is(sys_cmd::NEW_NATIVE_CHILD_WINDOW) => {
                if let Err(e) = self.new_native_child_window(cmd) {
                    tracing::error!("failed to create native child window: '{}'", e);
                }
            }
            _ if cmd.is(sys_cmd::SET_NATIVE_WINDOW_LAYOUT) => {
                if let Err(e) = self.set_native_windows_layout(cmd) {
                    tracing::error!("failed to set native window layout: '{}'", e);
                }
            }
